...
test("桌面端使用统一柔化令牌和路线栅格", async () => {
  ...
  assert.match(layout, /\.route-view\s*\{[^}]*grid-template-columns:\s*390px\s+minmax\(0,\s*1fr\)/s);
  ...
  assert.match(layout, /@media\s*\(min-width:\s*1600px\)[^{]*\{[^}]*\.route-view\s*\{[^}]*grid-template-columns:\s*410px/s);
});
...
test("路线左栏使用放大字号且地图点位卡保持紧凑", async () => {
  ...
  assert.match(layout, /\.route-stop-card\s*\{[^}]*right:\s*auto[^}]*width:\s*clamp\(360px,\s*30vw,\s*430px\)/s);
