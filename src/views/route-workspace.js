...
export function renderRouteWorkspace({ state, stops, selectedStop, copy, onSelect }) {
  ...
  list.innerHTML = stops.map((stop) => {
    ...
    return `<li><button type="button" data-stop-id="${stop.id}" class="${stop.id === selectedStop.id ? "active" : ""}"><span class="stop-no">${twoDigits(stop.routeOrder)}</span><span><strong>${localized(stop.name, state.language)}</strong><small>${localized(stop.role, state.language)} · ${stop.duration} min</small></span><small>${status}</small></button></li>`;
  }).join("");
